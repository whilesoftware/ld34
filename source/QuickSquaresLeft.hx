	...
	public function interpolate(sample_time:Float, _node:TimeNode, _type:TimeNodeType):Dynamic {
		...
		switch(_type) {
			...
			case TimeNodeType.color:
				if (left_position) {
					return FlxColorUtil.interpolateColor(value, _node.value, 1000, Std.int(fraction * 1000));
				}else{
					return FlxColorUtil.interpolateColor(_node.value, value, 1000, Std.int(fraction * 1000));
				}
		}
	}
}
...
//class TimeNodeColor extends TimeNode{
